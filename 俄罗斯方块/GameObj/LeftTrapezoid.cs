...
namespace 俄罗斯方块
{
    class LeftTrapezoid : BaseSquare
    {
        public LeftTrapezoid(int x, int y) : base(x, y, ConsoleColor.Yellow)
        {
            units = new List<List<Vector>>()
            {
                { new List<Vector>() { new Vector(-2, -1), new Vector(0, -1), new Vector(0, 0), new Vector(0, 1) } },
